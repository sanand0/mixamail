# http://github.com/BonsaiDen/twitter-text-python/blob/master/ttp.py
...
class Parser(object):
    ...
    # Modified for mixamail.com
    def format_tag(self, tag, text):
        '''Return formatted HTML for a hashtag.'''
        return '<a href="mailto:%s?subject=search%%20%s">%s%s</a>' \
                % (self.email, urllib.quote('#' + text.encode('utf-8')), tag, text)

    def format_username(self, at_char, user):
        '''Return formatted HTML for a username.'''
        return '<a href="mailto:%s?subject=search%%20%%40%s">%s%s</a>' \
               % (self.email, urllib.quote(user,''), at_char, user)
    ...
    def format_url(self, url, text):
        ...
        # return '<a href="mailto:%s?body=%s&subject=link">%s</a>' \
        #     % (self.email, urllib.quote(escape(url),''), text)
